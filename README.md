...
Depending on the script, the projects can generate:

```text
Clean and noisy signal plots
Filtered signal plots
Step response plots
Performance metric tables
Requirement validation reports
Pass/fail results
Simulink output response plots
Parameter sweep result tables
CSV result files
```
...
The goal of this repository is to practice a simple but realistic engineering workflow:
...
Select or reject candidate designs
```

Instead of only writing scripts that produce plots, this repository focuses on understanding how system behavior is measured and how engineering decisions can be made based on quantitative performance criteria.
...
```text
Adding the Simulink model file to the repository
Creating a main script that runs all projects
Saving all plots automatically
Organizing outputs into a results folder
Adding PID controller examples
Comparing proportional control with PID control
Adding automatic report generation
Improving visualization of passing and failing designs
```

## Author

Created by Atakan Erdogan as part of MATLAB and Simulink practice projects focused on signal processing, control-system analysis, validation, and engineering simulation workflows.
